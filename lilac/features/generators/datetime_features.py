...
class DatetimeFeatures(FeaturesBase):
    ...
    def transform(self, df):
        df[f"{self.input_col}_dt"] = pd.to_datetime(df[self.input_col], format=self.format_str)
        ...
        if self.include_ymd:
            ...
            df[f"{self.input_col}_month"] = (
                df[f"{self.input_col}_dt"]
                .apply(lambda x: x.month if not isinstance(x, type(pd.NaT)) else None)
                .astype(str)
            )
            df[f"{self.input_col}_day"] = (
                df[f"{self.input_col}_dt"]
                .apply(lambda x: x.day if not isinstance(x, type(pd.NaT)) else None)
                .astype(str)
            )
        df[f"{self.input_col}_day_name"] = (
            df[f"{self.input_col}_dt"]
            .apply(lambda x: x.day_name if not isinstance(x, type(pd.NaT)) else None)
            .astype(str)
        )
        ...
        df[f"{self.input_col}_is_weekend"] = df[f"{self.input_col}_dt"].apply(get_is_weekend).astype(str)
